...
void main() {
  ...
  final adminService =
      File('lib/services/administrative_request_service.dart').readAsStringSync();
  ...
  test('Field Mission is exposed as a separate request container', () {
    ...
    expect(requestScreen, contains("_buildFieldMissionForm(UserModel user, ThemeData theme)"));
  ...
  test('Field Mission submits directly to Firestore administrativeRequests collection', () {
    // Ensure submitFieldMission writes directly to Firestore with all required fields
    expect(adminService, contains("collection('administrativeRequests').doc()"));
    expect(adminService, contains("AdministrativeRequestCategory.fieldMission"));
    expect(adminService, contains("'siteName': siteName.trim()"));
    expect(adminService, contains("'missionDate': DateFormat('yyyy-MM-dd').format(date)"));
    expect(adminService, contains("'startTime': startTime"));
    expect(adminService, contains("'endTime': endTime"));
    expect(adminService, contains("'requiresReturnToOffice': requiresReturnToOffice"));
    expect(adminService, contains("'requiresCheckout': requiresCheckout"));
    expect(adminService, contains("ManagerApprovalChain.orderedIds"));
  });
