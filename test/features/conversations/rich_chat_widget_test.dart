...
class FakeRichChatRepository implements RichChatRepository {
  ...
  @override
  Future<({String body, List<ChatDraftFile> files, String? replyToMessageId})> loadDraft(String channelId) async =>
      (body: draft, files: <ChatDraftFile>[], replyToMessageId: null);
  @override
  Future<void> saveDraft(String channelId, String body, List<ChatDraftFile> files, {String? replyToMessageId}) async {
    draft = body;
  }
  @override
  Future<void> send(String channelId, {required String body, List<ChatDraftFile> files = const [], String? replyToMessageId}) async {
    sends++;
    if (failSend) throw const ChatFailure('storage_full');
  }
  @override
  Future<void> markRead(String channelId, String messageId) async { seen.add(messageId); }
  @override
...
void main() {
  testWidgets('failed durable enqueue keeps composer text and exposes retryable error', (tester) async {
    final repository = FakeRichChatRepository()..failSend = true;
    await tester.pumpWidget(MaterialApp(home: RichChatPage(
      repository: repository,
      channel: const RichChannel(id: 'c', name: 'فريق العمل', canPost: true),
      canReview: false,
    ...
    await tester.enterText(find.byKey(const Key('chat-composer')), 'رسالة محفوظة');
    await tester.pumpAndSettle();
  ...
  testWidgets('a foreground visible received message is marked seen', (tester) async {
    ...
    repository.snapshots.add(RichChatSnapshot(canPost: true, messages: [RichMessage(id: 'm', conversationId: 'c', senderUserId: 'other', body: 'أهلاً', sentAt: DateTime.now())]));
    await tester.pumpAndSettle();
  ...
  testWidgets('department channel info displays all department employees instead of 0 members', (tester) async {
    final repository = FakeRichChatRepository();
    await tester.pumpWidget(MaterialApp(home: RichChatPage(
      repository: repository,
      channel: const RichChannel(id: 'c', name: 'قناة قسم HR', kind: 'department', memberUserIds: [], canPost: true),
      canReview: true,
      attachmentBuilder: (_, attachment) => Text(attachment.fileName),
      pickAttachments: (_) async => [],
      voiceBuilder: (_, callback) => const SizedBox.shrink(),
  ...
  testWidgets('composer has attachment and voice options without camera video record button', (tester) async {
    final repository = FakeRichChatRepository();
    await tester.pumpWidget(MaterialApp(home: RichChatPage(
      repository: repository,
      channel: const RichChannel(id: 'c', name: 'القناة', canPost: true),
