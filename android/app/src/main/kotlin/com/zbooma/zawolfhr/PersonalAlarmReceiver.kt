...
class PersonalAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val oneShot = intent.getBooleanExtra(PersonalAlarmScheduler.EXTRA_ONE_SHOT, false)
        ...
        if (oneShot) {
            PersonalAlarmScheduler.markTriggered(context, userId)
        } else {
            runCatching {
...
/** Stops the alarm without requiring Flutter or the app activity to be alive. */
class PersonalAlarmStopReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != PersonalAlarmService.ACTION_STOP) return
        context.stopService(Intent(context, PersonalAlarmService::class.java))
        NotificationManagerCompat.from(context).cancel(PersonalAlarmService.NOTIFICATION_ID)
    }
}
...
object PersonalAlarmScheduler {
    ...
    private const val DATED_OWNER_PREFIX = "dated_owner_"
    ...
    fun schedule(context: Context, userId: String, hour: Int, minute: Int, message: String) {
        ...
        prefs.edit()
            ...
            .putBoolean("$ONE_SHOT_PREFIX$userId", false)
            .remove("$TRIGGER_PREFIX$userId")
        ...
        val calendar = Calendar.getInstance().apply {
            ...
            // SEG's work week is Saturday through Thursday.
            while (get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY) {
                add(Calendar.DAY_OF_YEAR, 1)
            }
        }
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        scheduleAlarmClock(context, userId, calendar.timeInMillis, hour, minute, message, false)
    }

    fun replaceDated(context: Context, ownerId: String, alarms: List<DatedAlarm>) {
        cancelDated(context, ownerId)
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val enabledUsers = prefs.getStringSet(USER_IDS, emptySet())?.toMutableSet()
            ?: mutableSetOf()
        ...
        for (alarm in alarms) {
            ...
            val alarmId = "$ownerId:${alarm.key}"
            enabledUsers.add(alarmId)
            editor
                .putBoolean("$ENABLED_PREFIX$alarmId", true)
                .putLong("$TRIGGER_PREFIX$alarmId", alarm.triggerAtMillis)
                .putString("$MESSAGE_PREFIX$alarmId", alarm.message)
                .putBoolean("$ONE_SHOT_PREFIX$alarmId", true)
                .putString("$DATED_OWNER_PREFIX$alarmId", ownerId)
            scheduleAlarmClock(
                context,
                alarmId,
        ...
        editor.putStringSet(USER_IDS, enabledUsers).apply()
    }
    ...
    private fun scheduleAlarmClock(
        ...
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        val operation = pendingIntent(context, alarmId, hour, minute, message, oneShot)
    ...
    fun cancelDated(context: Context, ownerId: String) {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val ids = prefs.getStringSet(USER_IDS, emptySet())?.toMutableSet() ?: mutableSetOf()
        val matching = ids.filter { prefs.getString("$DATED_OWNER_PREFIX$it", null) == ownerId }
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        val editor = prefs.edit()
        for (alarmId in matching) {
            alarmManager.cancel(pendingIntent(context, alarmId, 0, 0, "", true))
            ids.remove(alarmId)
            clearAlarm(editor, alarmId)
        }
        editor.putStringSet(USER_IDS, ids).apply()
    }

    fun markTriggered(context: Context, alarmId: String) {
        val prefs = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        val ids = prefs.getStringSet(USER_IDS, emptySet())?.toMutableSet() ?: mutableSetOf()
        ids.remove(alarmId)
        val editor = prefs.edit()
    ...
    private fun clearAlarm(editor: android.content.SharedPreferences.Editor, alarmId: String) {
        editor
            .remove("$ENABLED_PREFIX$alarmId")
            .remove("$HOUR_PREFIX$alarmId")
            .remove("$MINUTE_PREFIX$alarmId")
            .remove("$MESSAGE_PREFIX$alarmId")
            .remove("$TRIGGER_PREFIX$alarmId")
            .remove("$ONE_SHOT_PREFIX$alarmId")
            .remove("$DATED_OWNER_PREFIX$alarmId")
    }
    ...
    fun cancel(context: Context, userId: String) {
        ...
        prefs.edit()
            ...
            .apply()
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        alarmManager.cancel(pendingIntent(context, userId, 0, 0, "", false))
        context.stopService(Intent(context, PersonalAlarmService::class.java))
    }
    ...
    fun restoreEnabledAlarms(context: Context) {
        ...
        for (userId in userIds) {
            ...
            runCatching {
                if (prefs.getBoolean("$ONE_SHOT_PREFIX$userId", false)) {
                    val trigger = prefs.getLong("$TRIGGER_PREFIX$userId", 0)
                    if (trigger > System.currentTimeMillis()) {
                        scheduleAlarmClock(
                            context, userId, trigger, 0, 0,
                            prefs.getString("$MESSAGE_PREFIX$userId", null) ?: "منبه الدوام - ZaWolf HR",
                            true,
                        )
                    } else {
                        markTriggered(context, userId)
                    }
                } else {
                    schedule(
                        context,
