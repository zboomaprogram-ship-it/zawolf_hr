...
class _RequestsManagementScreenState extends State<RequestsManagementScreen> {
  ...
  Future<void> _showSendRequestForEmployeeDialog(UserModel manager) async {
    final usersSnapshot =
        await _db.collection('users').where('isActive', isEqualTo: true).get();
    final employees =
        usersSnapshot.docs.map(UserModel.fromFirestore).toList()
          ..sort((a, b) => a.displayName.compareTo(b.displayName));
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: const [
                            ...
                            DropdownMenuItem(
                              value: 'advance',
                              child: Text('طلب سلفة مالية'),
                            ),
                            DropdownMenuItem(
                              value: 'administrative',
                              child: Text('طلب إداري / مأمورية / تكليف'),
                            ),
                            DropdownMenuItem(
                              value: 'attendanceCorrection',
                              child: Text('طلب تصحيح حضور وانصراف'),
                            ),
                            DropdownMenuItem(
                              value: 'complaint',
                              child: Text('شكوى رسمية'),
                            ),
                            DropdownMenuItem(
                              value: 'resignation',
                              child: Text('طلب استقالة'),
                            ),
                          ],
                          onChanged:
                              saving
                        ...
                        if (requestKind == 'leave') ...[
                          DropdownButtonFormField<String>(
                            ...
                            items: [
                              DropdownMenuItem(
                                value: LeaveTypePolicy.normal,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                                    LeaveTypePolicy.normal,
                                  ),
                                ),
                              ),
                              DropdownMenuItem(
                                value: LeaveTypePolicy.casual,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                                    LeaveTypePolicy.casual,
                                  ),
                                ),
                              ),
                              DropdownMenuItem(
                                value: LeaveTypePolicy.sick,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                                    LeaveTypePolicy.sick,
                                  ),
                                ),
                              ),
                              DropdownMenuItem(
                                value: LeaveTypePolicy.unpaid,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                              ...
                              DropdownMenuItem(
                                value: LeaveTypePolicy.exam,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                                    LeaveTypePolicy.exam,
                                  ),
                                ),
                              ),
                              DropdownMenuItem(
                                value: LeaveTypePolicy.paternity,
                                child: Text(
                                  LeaveTypePolicy.arabicLabel(
                                    LeaveTypePolicy.paternity,
                                  ),
                                ),
                              ),
                            ],
                            ...
                            children: [
                              Expanded(
                                child: OutlinedButton.icon(
                                  ...
                                  label: Text(
                                    'من: ${DateFormat('yyyy-MM-dd').format(leaveStart)}',
                                  ),
                                  onPressed:
                                      saving
                                          ? null
                              ...
                              Expanded(
                                child: OutlinedButton.icon(
                                  ...
                                  label: Text(
                                    'إلى: ${DateFormat('yyyy-MM-dd').format(leaveEnd)}',
                                  ),
                                  onPressed:
                                      saving
                                          ? null
                            ...
                            children: [
                              Expanded(
                                child: OutlinedButton.icon(
                                  ...
                                  label: Text(
                                    DateFormat('yyyy-MM-dd').format(permDate),
                                  ),
                                  onPressed:
                                      saving
                                          ? null
                          ...
                          OutlinedButton.icon(
                            ...
                            label: Text(
                              'تاريخ اليوم المراد تصحيحه: ${DateFormat('yyyy-MM-dd').format(corrDate)}',
                            ),
                            onPressed:
                                saving
                                    ? null
                          ...
                          if (corrHasCheckIn) ...[
                            OutlinedButton.icon(
                              icon: const Icon(Icons.access_time, size: 16),
                              label: Text(
                                'وقت الحضور الصحيح: ${corrCheckIn.format(context)}',
                              ),
                              onPressed:
                                  saving
                                      ? null
                          ...
                          if (corrHasCheckOut) ...[
                            OutlinedButton.icon(
                              icon: const Icon(Icons.access_time, size: 16),
                              label: Text(
                                'وقت الانصراف الصحيح: ${corrCheckOut.format(context)}',
                              ),
                              onPressed:
                                  saving
                                      ? null
                          ...
                          OutlinedButton.icon(
                            icon: const Icon(Icons.calendar_today, size: 16),
                            label: Text(
                              'تاريخ آخر يوم عمل متوقع: ${DateFormat('yyyy-MM-dd').format(resLastDay)}',
                            ),
                            onPressed:
                                saving
                                    ? null
                        ...
                        saving
                            ...
                            : () async {
                              ...
                              if (chosenEmployee == null) {
                                messenger.showSnackBar(
                                  const SnackBar(
                                    content: Text('يرجى اختيار الموظف أولاً.'),
                                  ),
                                );
                                return;
                              }
                                ...
                                switch (requestKind) {
                                  case 'leave':
                                    ...
                                    final req = LeaveModel(
                                      ...
                                      locationId:
                                          chosenEmployee!.locationId.isNotEmpty
                                  ...
                                  case 'advance':
                                    final amount =
                                        double.tryParse(
                                          advanceAmountCtrl.text.trim(),
                                        ) ??
                                        0;
                                    if (amount <= 0) {
                                      throw Exception('أدخل مبلغ سلفة صحيحاً.');
                                    }
                                    ...
                                    await _advanceService.submitAdvanceRequest(
                                      advReq,
                                      chosenEmployee!,
                                    );
                                    break;
                                  ...
                                  case 'administrative':
                                    if (adminNotesCtrl.text.trim().isEmpty) {
                                      throw Exception(
                                        'يجب كتابة تفاصيل وملاحظات الطلب.',
                                      );
                                    }
                                  ...
                                  case 'complaint':
                                    if (complaintTitleCtrl.text
                                            .trim()
                                            .isEmpty ||
                                        complaintBodyCtrl.text.trim().isEmpty) {
                                      throw Exception(
                                        'يجب إدخال عنوان وتفاصيل الشكوى.',
                                      );
                                    }
  ...
  Future<void> _bulkReviewRequests(
    UserModel manager, {
    ...
    final adminIds =
        PendingRequestsService.instance.pendingAdministrativeIds.toList();
    final resIds =
        PendingRequestsService.instance.pendingResignationIds.toList();
                    ...
                    child: Column(
                      children: [
                        if (leaveIds.isNotEmpty)
                          _bulkSummaryRow('طلبات الإجازات', leaveIds.length),
                        if (permIds.isNotEmpty)
                          _bulkSummaryRow('طلبات الأذونات', permIds.length),
                        if (advIds.isNotEmpty)
                          _bulkSummaryRow('طلبات السلف', advIds.length),
                        if (adminIds.isNotEmpty)
                          _bulkSummaryRow('الطلبات الإدارية', adminIds.length),
                        if (resIds.isNotEmpty)
                          _bulkSummaryRow('طلبات الاستقالة', resIds.length),
                        const Divider(color: ZaWolfColors.surface03),
                        _bulkSummaryRow(
                          'الإجمالي المطلوب معالجته',
                          totalCount,
                          isBold: true,
                        ),
                      ],
  ...
  Future<void> _showInPlaceRequestAction(
                      ...
                      children: [
                        ...
                        TextButton.icon(
                          onPressed: () {
                            ...
                            _navigateToRecordCategory(context, record, tabs);
                          },
                    ...
                    child: SingleChildScrollView(
                      ...
                      >(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (!snapshot.hasData || !snapshot.data!.exists) {
                            return const Center(
                              child: Padding(
                                padding: EdgeInsets.all(24),
                                child: Text(
                                  'تعذر العثور على بيانات الطلب أو تم حذفه.',
                                  style: TextStyle(
                                    color: ZaWolfColors.textSecondary,
                                  ),
                                ),
                          ...
                          return _buildInPlaceCardForRecord(
                            doc: doc,
                            record: record,
                            reviewer: reviewer,
                            theme: theme,
  ...
  Widget _buildInPlaceCardForRecord({
  ...
  Widget _buildGenericRequestCard({
    ...
    return WolfCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildEmployeeHeader(title, record.employeeId, dept, theme),
          const SizedBox(height: 10),
          if (notes.isNotEmpty)
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: RequestTypeStyle.stateColor(
  ...
  Widget build(BuildContext context) {
    ...
    final actorScope = RequestActorScope(
      actorId: manager.uid,
      role: manager.role,
      isExecutive: manager.isExecutiveLeader,
      employeeCode: manager.employeeId,
    );
                  ...
                  Tooltip(
                    ...
                    child: ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(
                        backgroundColor: ZaWolfColors.success.withValues(
                          alpha: 0.18,
                        ),
  ...
  Widget _buildAdministrativeRequestsTab(
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final docs = _newestFirst(_visibleApprovalDocs(pendingDocs, reviewer));
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final isTarget = target.isNotEmpty && doc.id == target;
            return _buildAdministrativeRequestCard(
              doc: doc,
              reviewer: reviewer,
              theme: theme,
  ...
  Widget _buildAdministrativeRequestCard({
    required DocumentSnapshot<Map<String, dynamic>> doc,
    required UserModel reviewer,
    required ThemeData theme,
    bool isTarget = false,
        ...
        child: WolfCard(
          ...
          child: Column(
            ...
            children: [
              ...
              else if (_canActOnApproval(data, reviewer))
                _buildApprovalActions(
    ...
    return StreamBuilder<List<ResignationModel>>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder:
              (context, index) => _buildResignationCardFromModel(
                requests[index],
                reviewer,
                theme,
              ),
  ...
  Widget _buildResignationRequestCard({
    required DocumentSnapshot<Map<String, dynamic>> doc,
    required UserModel reviewer,
    required ThemeData theme,
  }) {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> _pendingStream(
    ...
    } else if (usesManagerChain && (isExecutive || isHr)) {
      query = query.where(
        'status',
        whereIn: const [
          'pending',
          ...
          children: [
            ...
            if (perm.isSubmittedAfterWorkStart)
              Container(
                ...
                child: const Text(
                  '⚠️ تم تقديم طلب التأخير بعد بداية وقت العمل (يتطلب مراجعة وقرار المدير)',
                  style: TextStyle(
                    color: ZaWolfColors.error,
                    fontSize: 11,
                    fontWeight: FontWeight.bold,
            ...
            Text(
              ...
              style: TextStyle(
                color:
                    perm.status == 'rejected' || perm.status == 'cancelled'
                        ? ZaWolfColors.error
                        : (perm.status == 'approved'
                            ? ZaWolfColors.success
                            : ZaWolfColors.primaryCyan),
                fontWeight:
                    perm.status == 'rejected' || perm.status == 'approved'
                        ? FontWeight.bold
                        : FontWeight.normal,
              ),
            ),
                ...
                color:
                    advance.status == 'rejected' ||
                        ...
                        : (advance.status == 'approved'
                            ...
                            : ZaWolfColors.primaryCyan),
                fontWeight:
                    advance.status == 'rejected' || advance.status == 'approved'
                        ? FontWeight.bold
                        : FontWeight.normal,
              ),
            ),
  ...
  Widget _buildSalaryDeductionsTab(
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (allItems.isEmpty && (reversalOnly || absenceOnly)) {
          return Column(
            children: [
              if (absenceOnly)
                Padding(
        ...
        return ListView(
          padding: const EdgeInsets.all(16),
          children: [
            if (!reversalOnly && !absenceOnly)
              _buildEarlyLeaveConsequenceQueue(reviewer, theme),
            if (absenceOnly) ...[
              WolfButton(
                onPressed: () => _syncAbsences(reviewer),
                text: 'مزامنة وتوليد غياب الشهر الحالي',
  ...
  Widget _buildEarlyLeaveConsequenceQueue(UserModel reviewer, ThemeData theme) {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: _cachedStream(
        'permissions|early-leave-consequences|${reviewer.uid}',
        _db
            .collection('permissions')
            .where('rejectionConsequence.status', isEqualTo: 'pending_hr')
            .limit(100),
      ),
      builder: (context, snapshot) {
        if (snapshot.hasError) {
        ...
        return Column(
          ...
          children: [
            ...
            ...docs.map((doc) {
              final data = doc.data();
              final consequence = Map<String, dynamic>.from(
                data['rejectionConsequence'] as Map? ?? const {},
              );
              final fraction =
                  (consequence['dayFraction'] as num?)?.toDouble() ?? 0;
              final hours =
                  ((consequence['requestedMinutes'] as num?)?.toInt() ?? 0) ~/
                  60;
              return Padding(
                padding: const EdgeInsets.only(bottom: 12),
                child: WolfCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        'مغادرة مبكرة $hours ساعة · خصم ${fraction.toStringAsFixed(2)} يوم',
                        textAlign: TextAlign.right,
                        textDirection: TextDirection.rtl,
                      ),
                      Text(
                        'تاريخ التنفيذ: ${data['requestDate'] ?? ''}',
                        textAlign: TextAlign.right,
                      ),
                      if ('${consequence['rejectionReason'] ?? ''}'.isNotEmpty)
                        Text(
                          'سبب الرفض: ${consequence['rejectionReason']}',
                          textAlign: TextAlign.right,
                        ),
                      const SizedBox(height: 10),
                      Row(
                        children: [
                          Expanded(
                            child: OutlinedButton(
                              onPressed:
                                  _isRequestBusy(doc.id)
                                      ? null
                                      : () => _reviewEarlyLeaveConsequence(
                                        doc.id,
                                        'rejected',
                                      ),
                              child: const Text('رفض الخصم'),
                            ),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
  ...
  Future<void> _reviewEarlyLeaveConsequence(
    String permissionId,
    String decision,
  ) async {
    await _withRequestGuard(permissionId, () async {
      await _attendanceGateway.reviewEarlyLeaveConsequence(
