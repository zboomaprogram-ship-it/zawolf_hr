...
class HrPeriodReportState {
  const HrPeriodReportState({
    required this.period,
    this.report,
    this.employeeId,
    this.error,
    this.exportedResourceId,
    this.loading = false,
  ...
  HrPeriodReportState copyWith({
    ...
    exportedResourceId:
        clearExport ? null : (exportedResourceId ?? this.exportedResourceId),
...
final class HrPeriodReportCubit extends Cubit<HrPeriodReportState> {
  HrPeriodReportCubit({
    required HrPeriodReportRepository repository,
    required UserModel reviewer,
  }) : _repository = repository,
       _reviewer = reviewer,
       super(HrPeriodReportState(period: HrReportPeriod.sevenDays())) {
    refresh();
  }
  final HrPeriodReportRepository _repository;
  final UserModel _reviewer;

  Future<void> selectPeriod(HrReportPeriod period) async {
    emit(state.copyWith(period: period, clearError: true));
    await refresh();
  }

  void selectEmployee(String? id) =>
      emit(state.copyWith(employeeId: id, clearEmployee: id == null));
  Future<void> export() async {
    emit(state.copyWith(exporting: true, clearError: true, clearExport: true));
    try {
      final resourceId = await _repository.export(
        period: state.period,
        employeeId: state.employeeId,
      );
      emit(state.copyWith(exportedResourceId: resourceId, exporting: false));
    } catch (error) {
      emit(state.copyWith(error: error, exporting: false));
    }
  }
  ...
  Future<void> refresh() async {
    ...
    emit(
      state.copyWith(loading: !cached, refreshing: cached, clearError: true),
    );
    try {
      ...
      final valid =
          ...
          report.employees.any((employee) => employee.id == state.employeeId);
      emit(
        state.copyWith(
          report: report,
          clearEmployee: !valid,
      ...
      emit(state.copyWith(error: error, loading: false, refreshing: false));
    }
  }
}
