...
class _ChatRequestFormPageState extends State<ChatRequestFormPage> {
  ...
  bool get _editable =>
      widget.request == null ||
      (widget.canReview && widget.request!.status == 'pending');
  @override
  void initState() {
    super.initState();
    _name = TextEditingController(text: widget.request?.name ?? '');
    _reason = TextEditingController(text: widget.request?.reason ?? '');
    _cubit = ChatRequestFormCubit(
      widget.repository,
      widget.request?.memberUserIds ?? [widget.repository.actorId],
    );
  }
  ...
  Future<void> _reject() async {
    ...
    controller.dispose();
    if (reason != null) {
      await _submit(decision: 'rejected', rejectionReason: reason);
    }
  }
  ...
  Widget build(BuildContext context) => Directionality(
    ...
    child: Scaffold(
      appBar: AppBar(
        title: Text(
          widget.request == null
              ? (widget.canReview ? 'إنشاء جروب جديد — HR' : 'طلب جروب جديد')
              : widget.canReview
              ? 'مراجعة طلب الجروب'
              : 'تفاصيل طلب الجروب',
        ),
      ),
      body: BlocBuilder<ChatRequestFormCubit, ChatRequestFormState>(
        bloc: _cubit,
        builder:
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (widget.request != null)
                      Text(
                        'حالة الطلب: ${chatRequestStatus(widget.request!.status)}',
                        style: Theme.of(context).textTheme.titleMedium,
                      ),
                    if (widget.request?.rejectionReason != null)
                      ChatFeedback(
                        text: 'سبب الرفض: ${widget.request!.rejectionReason}',
                      ),
                    Form(
                      key: _form,
                      child: Column(
                        children: [
                          ...
                          TextFormField(
                            controller: _reason,
                            enabled: widget.request == null && !state.busy,
                            maxLength: 1000,
                            minLines: 3,
                            maxLines: 6,
                    ...
                    OutlinedButton.icon(
                      onPressed:
                          !_editable || state.busy
                              ? null
                              : () async {
                                final selected = await pickChatUsers(
                                  context,
                                  widget.repository,
                      ...
                      label: Text(
                        'اختيار الزملاء (${state.members.length}/100)',
                      ),
                    ),
                      ...
                      children:
                          state.members
                              .map(
                                (member) => Chip(
                                  label: Text(
                                    member == widget.repository.actorId
                                        ? 'أنت'
                                        : member,
                                  ),
                                ),
                            ...
                            label: Text(
                              widget.request == null
                                  ? (widget.canReview
                                      ? 'إنشاء الجروب مباشرة'
                                      : 'إرسال الطلب إلى HR')
                                  : 'الموافقة وإنشاء الجروب',
                            ),
                          ),
                          if (widget.request != null)
                            OutlinedButton.icon(
                              onPressed: state.busy ? null : _reject,
                              icon: const Icon(Icons.close),
                              label: const Text('رفض مع السبب'),
                            ),
                        ],
                      ),
                  ],
...
String chatRequestStatus(String status) => switch (status) {
