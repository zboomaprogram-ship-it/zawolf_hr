...
class ChatSearchPage extends StatelessWidget {
  const ChatSearchPage({super.key, required this.repository, required this.channelId, required this.attachmentBuilder});
  ...
  Widget build(BuildContext context) => BlocProvider(create: (_) => ChatSearchCubit(repository, channelId), child: Directionality(textDirection: TextDirection.rtl, child: Scaffold(appBar: AppBar(title: const Text('البحث في رسائل الجروب')), body: BlocBuilder<ChatSearchCubit, ChatSearchState>(builder: (context, state) => Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 900), child: Column(children: [
    ...
    Expanded(child: ListView(children: [
      if (state.messages.isEmpty && !state.loading) ListTile(title: Text(state.query.isEmpty ? 'ابحث في سجل المحادثة' : state.complete ? 'لا توجد نتائج' : 'لا توجد نتائج في هذا الجزء من السجل')),
      for (final message in state.messages) Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(message.senderDisplayName, style: const TextStyle(fontWeight: FontWeight.bold)), SelectableText(message.isDeleted ? 'تم حذف الرسالة' : message.body), if (!message.isDeleted) ...message.attachments.map((attachment) => attachmentBuilder(context, attachment)), Text('${message.sentAt.toLocal()}', style: Theme.of(context).textTheme.labelSmall)]))),
      if (state.cursor != null) TextButton(onPressed: state.loading ? null : () => context.read<ChatSearchCubit>().search(state.query, more: true), child: const Text('متابعة البحث في الرسائل الأقدم')),
    ])),
  ])))))));
}
