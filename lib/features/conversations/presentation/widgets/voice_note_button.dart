...
class _VoiceNoteButtonState extends State<VoiceNoteButton>
  ...
  Widget _sheet(BuildContext context) => SafeArea(
    child: Padding(
        ...
        builder:
            (context, state) => Column(
              ...
              children: [
                ...
                if (state.file != null)
                  ChatLocalPlayer(file: state.file!, gateway: widget.gateway),
                Wrap(
                  children: [
                    if (state.file == null)
                      TextButton.icon(
                        ...
                        label: Text(state.recording ? 'إيقاف' : 'تسجيل'),
                      ),
                    if (state.file != null)
                      TextButton.icon(
                        onPressed: () async {
                          try {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('تعذر حفظ التسجيل في المسودة.'),
                                ),
                              );
                            }
                          }
