...
class ChatRequestFormCubit extends Cubit<ChatRequestFormState> {
  ChatRequestFormCubit(this.repository, List<String> members)
    : super(ChatRequestFormState(members: members));
  final RichChatRepository repository;
  void members(List<String> members) {
    if (!state.busy) emit(ChatRequestFormState(members: members));
  }

  Future<ChannelRequest?> submit({
    ...
    if ((request == null || decision == 'approved') &&
        (state.members.length < 2 || state.members.length > 100)) {
      emit(
        ChatRequestFormState(
    ...
    try {
      ...
      if (request == null) {
        final created = await repository.createRequest(
          name: name.trim(),
          reason: reason.trim(),
          memberUserIds: state.members,
        );
        if (canReview) {
          result = await repository.reviewRequest(
            created.id,
            decision: 'approved',
      ...
      if (!isClosed) {
        emit(
          ChatRequestFormState(members: state.members, error: error.toString()),
        );
      }
      return null;
    }
