...
class ChatMediaCubit extends Cubit<ChatMediaState> {
  ...
  Future<void> load() async {
    ...
    try { final file = await download(); if (!isClosed) emit(ChatMediaState(file: file)); }
    catch (_) { if (!isClosed) emit(const ChatMediaState(error: 'تعذر تحميل المرفق. أعد المحاولة.')); }
  }
}
